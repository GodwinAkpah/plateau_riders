...
class HomeController extends GetxController {
  ...
  /// This is the main action triggered by the 'Continue' button. It validates the user's
  /// selections and calls the booking service to search for available trips.
  Future<void> searchTripsForBooking() async {
    ...
    // 3. Convert the UI date format ('dd-MM-yyyy') to the API format ('yyyy-MM-dd').
    final parsedDate = DateFormat('dd-MM-yyyy').parse(selectedDate.value);
    final formattedDate = DateFormat('yyyy-MM-dd').format(parsedDate);

    // 4. Call the service and handle the response.
    ...
    if (response.status == "success" && response.data != null) {
      availableTrips.assignAll(response.data!);
      if (availableTrips.isEmpty) {
        Get.snackbar("No Trips Found", "No trips are available for the selected criteria.",
            backgroundColor: Colors.blueAccent, colorText: Colors.white);
      } else {
        Get.snackbar("Success", "${availableTrips.length} trips found!",
            backgroundColor: Colors.green, colorText: Colors.white);
        
        // NEXT STEP: Navigate to a new screen to show these trips.
        // For example: Get.toNamed(Routes.TRIP_SELECTION, arguments: availableTrips);
        print("Found trips: ${availableTrips.length}");
      }
    } else {
